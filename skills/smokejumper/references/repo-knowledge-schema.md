...
## Design system
# the repo's design source of truth, found by the DESIGN LEAD: token/theme file paths, color +
# spacing + type scales, component library / Storybook location, UI naming conventions. Durable +
# accumulative — the design lead writes/updates it from actual code inspection; the adapter scan
# records only candidate paths in ## Capabilities and NEVER writes this section. If no system
# exists, records the baseline the design lead is establishing. Append/update only.
...
# also carries adapter.skills.design.* (detected design skills) + functionalHealth + designPosture
# + adapter.designSystem (candidate source-of-truth paths the design lead then verifies)

## Design skills
# accumulative effectiveness tally per design skill — append/update only, NEVER regenerated by
# the adapter scan. Written by LESSONS LEARNED (Phase 7). Proven (score > 0) skills are surfaced
# by RECON as next-sprint recommendations. See lessons-learned.md for the gate-or-ship scoring.
...
**Write discipline:**
- `## Known traps`, `## What worked / What to avoid`, `## Framework improvements pending`,
  `## Design skills`, and `## Design system` are append/update-only. Prior entries survive every
  sprint (a design-system model is refreshed in place when the system changes, never wiped).
...
that section is recomputed each sprint. The scan must NEVER write `## Design skills` or
`## Design system`: both hold cross-sprint accumulated state and would be clobbered. `## Design skills`
is written only by LESSONS LEARNED; `## Design system` is written only by the DESIGN LEAD.
...
| `design_posture_set` | RECON derived `functionalHealth` + `designPosture` (counts in `detail`) |
| `design_skill_invoked` | Design lead invoked a design skill (skill name in `detail`) |
| `design_skill_outcome` | LESSONS LEARNED scored a design-skill use (pass/rework in `outcome`) |
