...
**Write discipline:**
- `## Known traps`, `## What worked / What to avoid`, `## Framework improvements pending`, and
  `## Design skills` are append/update-only. Prior entries survive every sprint.
...
into `## Capabilities` — safe, because that section is recomputed each sprint. The scan must NEVER
write the `## Design skills` section: that section holds cross-sprint accumulated state and would be
clobbered. `## Design skills` is written only by LESSONS LEARNED.
