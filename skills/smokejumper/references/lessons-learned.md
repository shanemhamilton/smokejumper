...
**`## Design skills`** — score the design skills the design lead invoked this sprint (each a
`design_skill_invoked` event in `sprint-log.jsonl`) and emit a `design_skill_outcome` event per
skill. This section is **owned here**: the adapter scan writes design-skill *detection* into
`## Capabilities`, but NEVER this tally — it holds cross-sprint state. Append/update only; never
regenerate.

Scoring — the **gate-or-ship proxy** (the design-review gate is adapter-conditional and usually
absent, so a gate-passed signal alone would leave every skill at 0 and the recommendation loop
inert):
- **Gate present** (`adapter.gate.designReviewer` non-null): +1 if it passed for the unit the
  skill informed; 0 if it required changes.
- **No gate** (common): +1 if the informed unit reached INTEGRATE **this sprint** with no
  `gate_failed` on that surface this sprint; 0 if design rework was needed this sprint. Score on
  what is knowable now — do not reference a "next sprint" that hasn't happened when Phase 7 fires.
  If a later sprint redoes the same surface (a fresh `design_skill_invoked` on it), that sprint
  revises this skill's row down by one; the tally is update-able, so the signal self-corrects
  without a forward-looking clause at scoring time.

Increment `Invoked` each use; increment `Clean ship` on +1 or `Rework` on 0; `Score` is the running
sum. Proven (score > 0) skills are what RECON recommends next sprint. Tally shape:

```markdown
## Design skills
# Effectiveness tally — accumulative, never regenerated by the adapter scan.

| Skill | Invoked | Clean ship | Rework | Score |
|---|---|---|---|---|
| design-review | 3 | 3 | 0 | +3 |
```
...
1. **Generalize AND scrub — mandatory before anything leaves the machine.** The portability
   ...
   hard gate. A learned design-skill recommendation graduates as a **generalized heuristic**
   (e.g. "detect and prefer a proven designer's-eye QA skill"), never the environment-specific
   skill name (`gstack`, `frontend-design`, etc.) or the per-repo `## Design skills` tally —
   those stay local.
...
- `design_skill_outcome` — when a design skill invoked this sprint is scored into the
  `## Design skills` tally (gate-or-ship proxy; pass/rework in the event `outcome`)
