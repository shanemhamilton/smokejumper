...
- **Real design-skill detection + invocation across Claude and Codex.** The scan detects installed design skills/plugins (`adapter.skills.design.review|consult|a11y|frontend`) by scanning `~/.claude/skills/`, `~/.claude/skills/gstack/`, and `~/.claude/plugins/installed_plugins.json`, recording them in `## Capabilities` and the banner. The design lead's skill table now resolves through these adapter keys instead of illustrative names, with a runtime-parity rule: native Skill tool under Claude Code, inline `SKILL.md` read under Codex (Skill tool absent).
